...
bool ECellEngine::IO::ClearDataStateCommand::DecodeParameters(const std::vector<std::string>& _args)
{
	if ((unsigned char)_args.size() != nbArgs)
	{
		ECellEngine::Logging::Logger::LogError("ClearDataStateCommand Failed: Wrong number of arguments. Expected %llu, got %u.", nbArgs, _args.size());
		return false;
	}
	...
	{
		ECellEngine::Logging::Logger::LogError("ClearDataStateCommand Failed: Could not convert first argument \"%s\" to an integer to represent the ID of a simulation", _args[1].c_str());
		return false;
	}
...
bool ECellEngine::IO::ClearDataStateCommand::Execute()
{
	std::pair<bool, std::vector<std::unique_ptr<Core::Simulation>>::iterator> simuSearch = receiver.FindSimulation(args.simulationID);

	if (!simuSearch.first)
	{
		ECellEngine::Logging::Logger::LogError("ClearDataStateCommand Failed: Could not find simulation with ID \"%llu\".", args.simulationID);
		return false;
	}

	simuSearch.second->get()->GetDataState().Clear();
